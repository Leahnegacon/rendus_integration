...
async function chargerPays() {
  conteneurapi.innerHTML = '<p class="loading">Chargement des pokémons...</p>';

  try {
    const reponse = await fetch('https://pokeapi.co/api/v2/pokemon?limit=20');
    const pokémons = await reponse.json();

    conteneurapi.innerHTML = '';
    pokémons.results.forEach((p) => {
      const carte = document.createElement('article');
      carte.classList.add('carte');
      carte.innerHTML = `
        <h3>${p.name}</h3>
    ...
    conteneurapi.innerHTML = '<p class="error">Impossible de charger les pokémons.</p>';
  }
}
